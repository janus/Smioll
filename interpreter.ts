import { Instruction, AST, Mem, Value, FunctionCall, SmolFunction } from "./type";
...
export function interpreter(instruction: AST | Instruction, mem: Mem): Value {
    ...
    const isPrimitive = instruction.type === "string" || instruction.type === "boolean" || instruction.type === "number";
...
export function interpretFnCall(call: FunctionCall, mem: Mem): Value {
    if(call.function === "=") {
        ...
        if(identifier instanceof Array) throw new Error("Syntax error on assignment")
